 ...
 * File Viewer example
 */
public class FileViewerWindow  extends Composite {
  ...
  private final String[] tableTitles = new String[] {
      FileViewerWindow.getResourceString("table.Name.title"),
      FileViewerWindow.getResourceString("table.Size.title"),
      FileViewerWindow.getResourceString("table.Type.title"),
      FileViewerWindow.getResourceString("table.Modified.title") };
  ...
  ////////////////////////////Constructor!!!!///////////////////
  public FileViewerWindow(Composite parent, int style, Shell parentShell1) {
	  
	  super(parent, style);	
	  
	  this.parentShell = parentShell1;
		
		// TODO Auto-generated constructor stub
		
		this.createContents(parent, parentShell); 
	}
...
//   */
   ...
   */
  private void createContents(Composite parent, Shell parentShell) {
//    this.setText(getResourceString("Title", new Object[] { "" }));
//    shell.setImage(iconCache.stockImages[iconCache.shellIcon]);
    Menu bar = new Menu(parent);
    		//(this, SWT.BAR);
    parentShell.setMenuBar(bar);
    createFileMenu(bar);
    createHelpMenu(bar);
    ...
    createComboView(this, gridData);
    gridData = new GridData(GridData.HORIZONTAL_ALIGN_FILL);
    gridData.horizontalSpan = 2;
    createToolBar(parentShell, gridData);
    ...
    numObjectsLabel = new Label(this, SWT.BORDER);
    gridData = new GridData(GridData.FILL_HORIZONTAL
        | GridData.VERTICAL_ALIGN_FILL);
    gridData.widthHint = 185;
    numObjectsLabel.setLayoutData(gridData);

    diskSpaceLabel = new Label(this, SWT.BORDER);
    gridData = new GridData(GridData.FILL_HORIZONTAL
        | GridData.VERTICAL_ALIGN_FILL);
    gridData.horizontalSpan = 2;
...
//    });
  ...
  private void createTreeView(Composite parent) {
    ...
    treeScopeLabel.setText(FileViewerWindow
...
//    shell.setCursor(iconCache.stockCursors[iconCache.cursorDefault]);
  ...
  private void treeRefreshItem(TreeItem dirItem, boolean forcePopulate) {
    ...
    File[] subFiles = (dir != null) ? FileViewerWindow.getDirectoryList(dir)
        : null;
  ...
  void notifySelectedDirectory(File dir) {
     ...
     */
    parentShell.setText(getResourceString("Title",
  ...
  void handleDeferredRefresh() {
    ...
    parentShell.setCursor(iconCache.stockCursors[iconCache.cursorWait]);
    ...
    parentShell.setCursor(iconCache.stockCursors[iconCache.cursorDefault]);
  }
  ...
  void doDefaultFileAction(File[] files) {
    ...
    if (file.isDirectory()) {
      ...
      if (!Program.launch(fileName)) {
        MessageBox dialog = new MessageBox(parentShell, SWT.ICON_ERROR
            | SWT.OK);
        dialog
            .setMessage(getResourceString(
                "error.FailedLaunch.message",
                new Object[] { fileName }));
        dialog.setText(parentShell.getText());
        dialog.open();
      }
    }
  }
  ...
  private void dropTargetHandleDrop(DropTargetEvent event, File targetFile) {
    ...
    progressDialog = new ProgressDialog(parentShell,
      ...
      while (!progressDialog.isCancelled()) {
        if (copyFileStructure(source, dest)) {
          ...
          if (event.detail == DND.DROP_MOVE && (!isDragging)) {
            ...
            MessageBox box = new MessageBox(parentShell, SWT.ICON_ERROR
  ...
  private void dragSourceHandleDragFinished(DragSourceEvent event,
    ...
    if (progressDialog == null)
      progressDialog = new ProgressDialog(parentShell, ProgressDialog.MOVE);
  ...
  private void workerExecute() {
    ...
    display.syncExec(new Runnable() {
      public void run() {
        tableContentsOfLabel.setText(FileViewerWindow.getResourceString(
  ...
  public void initResources(Display display) {
    if (stockImages == null) {
      ...
      for (int i = 0; i < stockImageLocations.length; ++i) {
        ...
        if (image == null) {
          freeResources();
          throw new IllegalStateException(FileViewerWindow
